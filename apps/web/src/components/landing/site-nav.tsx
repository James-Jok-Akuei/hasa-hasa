 ...
 *
 *  Pinned to the top of the viewport. Over the hero it keeps the ambient
 *  colour of the current slide (bandColor), crossfading with the slideshow;
 *  once the page scrolls past that, the band turns to frosted white so the
 *  capsule still reads against whatever section is underneath.
 *
 *  Fixed elements leave no gap behind them, so the component renders its own
 *  spacer of the measured height — the pages below need no padding of their
 *  own, and nothing shifts when the nav resizes at a breakpoint. */
export function SiteNav({
  ...
  return (
    <>
      <header
        ref={headerRef}
        className={`fixed inset-x-0 top-0 z-40 px-4 pb-2 pt-4 transition-colors ease-in-out sm:px-6 lg:px-[8vw] ${
          scrolled
            ? "bg-white/70 shadow-[0_10px_30px_-24px_rgba(0,0,0,0.8)] backdrop-blur-xl"
            : ""
        }`}
        style={{
          // Past the hero the ambient colour would be a stripe over unrelated
          // sections, so it yields to the frosted background.
          backgroundColor: scrolled ? undefined : bandColor,
      ...
      >
        ...
        {open ? (
          <div className="fixed inset-0 z-50 flex animate-fade-up flex-col bg-black/90 backdrop-blur-xl lg:hidden">
      ...
      {/* Holds the space the fixed header vacated. The measured height only
        arrives after mount, so the classes carry the first paint. */}
      <div
        aria-hidden
        className="h-[4.25rem] sm:h-[5.5rem]"
